...
module.exports = {
    ...
    run: async (client: ClientCustom, message: Message, args) => {
        ...
        let data = await axios.get('https://nekos.life/api/v2/img/lizard');
        let embed = new RichEmbed().setImage(data.data.url).setTitle(`:lizard::lizard::lizard::lizard::lizard::lizard:`).setFooter('Provided by nekos.life').setColor(roleColor);
        msg.edit(embed);
    }
}